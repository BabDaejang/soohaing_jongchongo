...
## 리팩토링 3 — 페이즈 2 실행 병렬화 + 진실성 스테이지 재편(ISBN 정규화·팩트시트 준비 분리) + respread 계측 (2026-07-14, 배치 1~3) ✅ 완료(자동 구간) / ⏳ 브라우저·실 LLM 실검증 사용자 대기

> 상세 진행·인계는 `docs/리팩토링_3_프로그레스.md`, 설계 판단은 `docs/DECISIONS.md`(2026-07-14 리팩토링 3 배치 1~3). 실행 지시서는 `docs/리팩토링_3.md`(v1, 배치 3개). 마이그레이션 변경 및 스키마 변경 없음 (0013 불변).

- **배치 1**(`useSequentialRun` 동시성 및 다단 연쇄): `useSequentialRun` 훅을 opt-in 동시성 풀(`concurrency?: number`, 1~3 클램프, 기본 1)로 확장. Promise 슬라이딩 풀, 일시정지(pause)/긴급중단(stop, 모든 in-flight 정산 후 1회 aborted), 서킷 브레이커(완료 순서 기준), 다단 연쇄 일반화(RunPlan 체인, 상한 5스테이지), 재시도 계약(StepResult `retryable`, 백오프 2초→6초 재시도) 구현. `lib/hooks/run-pool.ts` 순수 스케줄링 코어 분리 및 단위 테스트 14건 작성. `lib/llm/` 오류 HTTP status 보존.
- **배치 2**(진실성 파이프라인 3스테이지 분리): 진실성 파이프라인을 식별(S1, ISBN 정규화) → 팩트시트 준비(S2, 책 단위) → 대조(S3, 제출물 단위) → 채점(S4)의 4단 연쇄로 개편. S2가 고유 책 단위로 동작하므로 병렬 실행 시 `factsheets_owner_isbn` 고유 인덱스 충돌 및 이중 API 수집을 원천 차단. `verifyAuthenticityOne`에서 생성/수집 경로를 제거하고 대조만 하도록 단순화. `tests/authenticity.test.ts`에 8건의 신규 단위 테스트 추가.
- **배치 3**(페이즈 2 UI 결선 및 respread 계측): 페이즈 2 UI에 동시성 선택(1~3건, 기본 2) Select UI 및 안내 추가. LLM API 오류(429/503/529)의 retryable 배선 완료. `recomputeAndSave`에서 공간 소진으로 인한 전체 재배치(`respread: true`) 발생 시 `score.respread` 감사 로그 기록 및 터미널/UI 메시지 경고 표기 계측 구현.

### 배포 후 사용자 체크리스트 (에이전트 미수행 — OAuth·LLM·외부 API 키·과금 필요)

1. **채점 실행 4스테이지 연쇄**: [채점 실행] 시 터미널에 식별(S1, 알라딘 ISBN 로그) → 팩트시트 준비(S2, 책 단위 재사용/생성) → 대조(S3, 원문 대조) → 채점(S4) 연쇄 흐름 실시간 확인.
2. **책 단위 팩트시트 준비 및 중복 생성 없음**: 같은 책을 인용한 학생이 2명 이상이어도 S2 단계에는 해당 책이 1건만 등장하며 중복 생성/수집 충돌이 발생하지 않는지 확인.
3. **동시 2·3건 병렬 실행**: "동시 처리" 드롭다운에서 2건 또는 3건을 선택하고 실행 시, 진행 로그가 완료 순서대로 교차 출력되며 건별 소요 시간(`(n.n초)`)이 출력되는지 확인.
4. **일시정지/긴급중단/증분 재실행**: 병렬 실행 중 일시정지(신규 투입 중단, in-flight 완료 대기) → 재개 및 긴급중단(처리분 반영 후 aborted 종료)이 올바르게 작동하며, 재실행 시 이미 성공한 단계/건이 제외되는 증분 동작 확인.
5. **429 레이트리밋 재시도**: 동시 3건 실행 등 429 API 오류 유발 상황에서 터미널에 `재시도 n/2` 백오프 로그가 찍힌 뒤 성공(또는 최종 실패)으로 수렴하며 서킷 브레이커가 오발되지 않는지 확인.
6. **기존 훅 소비자 하위 호환**: 수합·생기부·보강·관리자 검증 등 기존 훅 사용처가 concurrency 미전달(=1)로 인해 기존과 완전히 동일하게(직렬) 정상 동작하는지 확인.
7. **재배치(respread) 경고 관찰**: 채점 또는 재계산 결과, 점수 공간 부족으로 전체 재배치 발생 시 터미널 및 결과 안내에 `⚠ 표시 점수 전체 재배치 발생` 경고가 출력되는지 및 audit_logs에 `score.respread` 로그가 기록되는지 확인.

## 리팩토링 4 — 계획 수립 (2026-08-20, 전면 검토·UX 재편 실행 지시서) ✅ 계획 완료 / ⏳ 배치 실행 대기

> 사용자 보고: "구조가 꼬여 의도대로 진행되지 않는다" — ① 수합이 여러 파일의 학생 **합집합 명단**을 만들지 못함 ② 매칭 수동 수정의 반영 신뢰 부족 ③ 평가는 선택이어야 함 ④ 팩트시트 독립 관리 ⑤ 전 단계 AI 부여 가능성·할루시네이션 ⑥ 채점 결과 확인·수정이 비직관(작업결과표에 숨음, 점수·등급·내용 즉시 수정 필요) ⑦ 독서 외 활동(예: JAYUL — 협업·피드백·희생·봉사 강조) 유연 대응 + MD 브리프 업로드/AI 협업 작성/웹 직접 수정.

- [x] 전면 코드 검토·원인 진단: 명단이 매칭의 선행 조건(prepareMatching이 명단 없으면 파일명·LLM 매칭 스킵, 학생 자동 생성은 column 학번뿐) → 개별 파일의 신규 학생이 영원히 미발견. 작업결과표는 `student_id NOT NULL`만 조회 → 미귀속 제출물 비가시. 기준별 채점 결과(evaluations.scores+evidence) 표시·수정 UI 전무(쓰기 service role 전용). 전역 내비 부재로 /factsheets가 페이즈 2 링크 종속. 생성 프롬프트에 활동 맥락 주입 자리 없음.
- [x] `docs/리팩토링_4.md`(v1) 작성 — 배치 7개 실행 지시서(OPUS 단독 실행 가능 수준의 공통 프롬프트·공유 계약·불변 규칙·배치별 단계·완료 기준·배포 후 체크리스트):
  배치 1 전역 내비+평가 '(선택)' 표기 / 배치 2 수합 **발견 스테이지**(명단 무관 식별값 추출+토큰 실존 검증 → 교사 일괄 승인 로스터 부트스트랩, 마이그레이션 없음) / 배치 3 작업결과표 허브화(미귀속 배너·매칭 배지·[학생 이동]·즉시 피드백) / 배치 4 평가 리뷰·수정(0014: evaluations.origin='teacher' 이력 insert + 목표 등급→보정 점수 헬퍼, INV-6 불변) / 배치 5 생기부 브리프(0015: prompt_profiles.brief_md + 프롬프트 주입·미리보기) / 배치 6 브리프 AI 초안·다듬기(자동 반영 금지) / 배치 7 SPEC/DATA_MODEL 개정+정리+최종 검증.
- [x] DECISIONS 2026-08-20 기록(진단·설계 골자 P-1~P-9). 배치별 상세 판단은 각 배치 실행 시 기록.
- typecheck·build: **해당 없음** — 이 세션은 문서만 작성(애플리케이션 코드 무변경).

### 다음 세션 인계

- 실행은 `docs/리팩토링_4.md` 사용법대로 **배치 1부터, 배치마다 새 대화**: "docs/리팩토링_4.md의 공통 프롬프트를 따르고 배치 N을 실행해". 진행 기준 문서 `docs/리팩토링_4_프로그레스.md`는 배치 1이 신설.
- 마이그레이션은 배치 4(0014)·배치 5(0015)만. 지시서와 코드가 어긋나면 코드가 기준(지시서 명시).
- 사용자 검토 포인트 3건(지시서 P-1·P-3·P-4): 발견 학생은 자동 생성이 아니라 **일괄 승인 경유**, 평가 수정은 **이력 보존 insert**, 등급은 직접 수정 대신 **목표 등급→보정 점수 계산**(INV-6 유지). 이견 시 배치 1 시작 전에 지시서를 수정할 것.

## 리팩토링 4 — 배치 1 (전역 내비게이션 + IA 재편) — 2026-08-20 ✅ 완료

> 상세 진행·인계는 `docs/리팩토링_4_프로그레스.md`, 설계 판단은 `docs/DECISIONS.md`(2026-08-20 리팩토링 4 배치 1). 실행 지시서는 `docs/리팩토링_4.md`(v1, 배치 7개). **데이터·스키마·파이프라인 무변경**(마이그레이션 0013 불변).

- [x] `components/global-nav.tsx`(서버 컴포넌트) + `lib/nav.ts`(순수 분기) 신설 — 승인 사용자 전 화면 상단 고정 헤더(프로젝트·팩트시트·계정·관리자 + 로그아웃), 비로그인·미승인은 렌더 자체를 생략(P-6).
- [x] 페이즈 2 "평가 (선택)" 표기 + 페이즈 3 안내 문구 — 생기부 생성이 평가와 독립임을 코드로 확인하고(records 액션·컨텍스트에 점수·등급 참조 0건) UI 표기를 코드 현실에 맞춤(P-5).
- [x] 팩트시트 위상 정리 — Phase2Panel 링크 "도서팩트시트 관리(전역) →", `/factsheets` 부제에 계정 단위 문구.
...
## 리팩토링 4 — 배치 7 (SPEC/DATA_MODEL 개정 + 코드 정리 + 최종 검증) — 2026-08-21 ✅ 완료 · **리팩토링 4 종료**

> 상세는 `docs/리팩토링_4_프로그레스.md`(배치 7 기록 + 종료 요약표), 판단은 `docs/DECISIONS.md`(2026-08-21 배치 7). **새 기능 없음 · 마이그레이션 없음**(0015 불변).

- [x] **SPEC 개정 9개 지점**(각주 "(2026-08-21 리팩토링 4, 사용자 승인)"): 4절 전역 내비·페이즈 2 "(선택)" / 5.2절 발견 스테이지 / 5.4절 수정 허브 / 6절 평가 선택화·교사 채점 수정·목표 등급 헬퍼 / 7.5절 작성 브리프 / 8절 작업결과표 허브.
- [x] DATA_MODEL 8절 보완(발견의 raw_* 재사용·identity_source 의미 확장·match_method 무제약), 9·12절 검수. README 기능 개요 신설 + 마이그레이션 목록 0001~0015 + repair 절차.
- [x] 코드 정리 — `any` **11건 전부 제거**(공용 `readClaimTitle` 신설로 jsonb 파고들기 통합), 미사용 심볼 3건, 이스케이프 2건. 다인용 PDF 귀속 라벨 정합. 구 라우트 4종은 이미 redirect 전용이라 무변경.
- [x] 최종 검증 — `npm test` **292/292** · `tsc --noEmit` 무오류 · **`eslint` 0 errors 0 warnings**(리팩토링 3 이후 처음) · `next build` 성공 · **INV-4 번들 9패턴 0건** · INV-1/6 코드·pg 확인 · 마이그레이션 원격 15 = 로컬 15.
- [x] `docs/SMOKE_TEST.md`에 [D] 리팩토링 4 브라우저 체크리스트 12항목 수록.

### 리팩토링 4 전체 요약 (배치 1~7, 2026-08-20~21)

- 사용자 보고 7건 전부 대응: **합집합 명단**(발견 스테이지 + 교사 일괄 승인) · **매칭 수정 신뢰**(작업결과표 허브·즉시 피드백·대시보드 revalidate) · **평가 선택화** · **팩트시트 독립**(전역 내비) · **전 단계 AI 가드**(토큰 실존 대조·무자동반영) · **채점 확인·수정**(origin='teacher' 이력 + 목표 등급 헬퍼) · **활동 유연 생기부**(brief_md + AI 협업).
- 지표: 테스트 **224 → 292**, eslint **14 problems → 0**, 마이그레이션 **0013 → 0015**(0014 evaluations.origin · 0015 brief_md).
- INV-1~6 전부 불변 — 특히 등급 직접 입력 경로 부재(INV-6)와 발견분의 자동 학생 생성 부재를 배치마다 재확인했다.
- **남은 것은 브라우저 실검증**(SMOKE_TEST [D] 12항목) — OAuth·LLM 과금 필요.
